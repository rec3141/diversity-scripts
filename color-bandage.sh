...
# uses bbmap's sendsketch / https://sourceforge.net/projects/bbmap/
...
for file in $GFA; do
	...
	grep '^S'  $f1.gfa | cut -f2,3 | sed 's/^\([0-9]\)/>\1/' | tr $'\t' $'\n' > $f1.assembly.fasta
   	...
   	#run sendsketch
   	sendsketch.sh in=$f1.assembly.fasta address=nt mode=sequence records=1 out=$f1.sketch.tsv ow=t
    ...
    rm $f1.assembly.fasta $f1.sketch.tsv $f1.names.tsv $f1.colors.tsv $f1.taxcolors.csv $f1.nodes.csv
done;
